...
login: ## login to ghcr.io using a personal access token (PAT)
	@if [ -z "$(CR_PAT)" ]; then\
		...
		echo $(CR_PAT) | docker login ghcr.io -u RobertoFioravanti --password-stdin;\
	fi

tag: ## tag docker image to ghcr.io/RobertoFioravanti/project:latest
	@docker tag project:latest ghcr.io/RobertoFioravanti/project:latest

push: tag ## docker push to container registry (ghcr.io)
	@docker push ghcr.io/RobertoFioravanti/project:latest

scan: ## scan the docker image for vulnerabilities
	@trivy image project:latest --scanners vuln --format table --severity  CRITICAL,HIGH --ignorefile .trivyignore --output "trivyreport.txt"
